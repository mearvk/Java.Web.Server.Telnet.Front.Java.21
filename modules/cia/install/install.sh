#!/usr/bin/env bash
# California CIA module installer — follows the NWE Unified Installation Contract.
set -euo pipefail
...
if [[ "${NWE_CONFIGURE_FIREWALL:-0}" == "1" ]]; then
  ...
  nwe_ensure_ufw
  if [[ -n "${NWE_TRUSTED_IP:-}" ]]; then nwe_run_privileged ufw allow from "$NWE_TRUSTED_IP" to any port 49211 proto tcp; else nwe_run_privileged ufw allow 49211/tcp; fi
else echo "  Firewall unchanged. Set NWE_CONFIGURE_FIREWALL=1 to manage it."; fi
nwe_verify_path "$MOD_ROOT"
echo "[OK] California CIA module installation complete. Backend port: 49211"
